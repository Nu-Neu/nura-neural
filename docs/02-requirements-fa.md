...
۳. هر آیتم دریافت‌شده با استفاده از `url_hash` (SHA-256) برای تکراری بودن بررسی شود
۴. اگر تکراری بود، آیتم Skip شود و با دلیل `DUPLICATE_URL` لاگ شود
۵. عکس HTML خام در Azure Blob Storage (Hot Tier) ذخیره شود با مسیر: `raw/{source_id}/{date}/{url_hash}.html`
۶. متادیتا (عنوان، متن، نویسنده، تاریخ انتشار) با **GPT-5-nano** استخراج شود
۷. اگر استخراج با GPT-5-nano شکست خورد یا کیفیت پایین بود، به GPT-4o-mini برای پردازش مجدد ارجاع شود
۸. زبان محتوا با استفاده از `langdetect` تشخیص داده شود و در فیلد `language_detected` ذخیره شود
۹. اگر محتوا به زبان فارسی یا عربی باشد، با GPT-5-nano به انگلیسی ترجمه شود و در فیلدهای `title_en`, `body_en` ذخیره شود
۱۰. داده استخراج‌شده در جدول `items` با وضعیت `PENDING_ANALYSIS` درج شود
...
- ADR-000۲: استفاده از GPT-5-nano برای کاهش هزینه، fallback به GPT-4o-mini
- ADR-000۴: ذخیره دوگانه (original + translation) برای تأیید کاربر
...
۱. یکپارچه‌سازی Twitter API v2 از طریق n8n (احراز هویت OAuth 2.0)
۲. رصد ۲۰۰ اکانت از Source Bible (روزنامه‌نگاران شهروندی تأییدشده، فعالان)
۳. رصد هشتگ‌ها: `#IranProtests`, `#MahsaAmini`, `#OpIran` (قابل تنظیم)
۴. فاصله زمانی polling: هر ۵ دقیقه برای اکانت‌های اولویت‌دار، ۱۵ دقیقه برای بقیه
۵. توییت‌ها با استفاده از `tweet_id` (کلید اصلی) dedup شوند
۶. پیوست‌های رسانه‌ای (تصاویر/ویدیوها) به Azure Blob Storage دانلود شوند
۷. متادیتای توییت استخراج شود: `author_handle`, `verified_status`, `retweet_count`, `like_count`, `timestamp`
۸. اگر توییت به فارسی یا عربی باشد، با GPT-5-nano ترجمه شود
...
۱. **Dedup بر اساس URL:** محاسبه SHA-256 hash از URL استاندارد (حذف UTM params، trailing slash)
۲. **Dedup بر اساس محتوا:** محاسبه SimHash از `title + body_text` (امضای ۱۰۲۴ بیتی)
۳. اگر شباهت SimHash ≥۹۵٪ با آیتم موجود در ۲۴ ساعت گذشته، علامت بزن `REPRINT`
۴. Reprintها به آیتم اصلی لینک شوند از طریق `original_item_id` (FK به `items.id`)
۵. Reprintها Trust Scoring را trigger نکنند (امتیاز را از اصلی به ارث ببرند)
۶. منطق Dedup **قبل از** استخراج GPT-5-nano اجرا شود تا هزینه API صرفه‌جویی شود
...
۱. تشخیص زبان با استفاده از کتابخانه `langdetect` (Python)
۲. زبان‌های پشتیبانی‌شده: انگلیسی (EN)، فارسی (FA)، عربی (AR)
۳. آیتم‌هایی که در زبان‌های پشتیبانی‌شده نیستند با وضعیت `LANGUAGE_MISMATCH` علامت بزن
۴. آیتم‌های دورریخته‌شده با کد زبان تشخیص‌داده‌شده (ISO 639-1) لاگ شوند
۵. آیتم‌های دورریخته‌شده در جدول `items` ذخیره **نشوند** (فیلتر قبل از درج)
...
۱. **پیکربندی Miniflux Rule Engine:**
...
۴. **تنظیم دوره‌ای (Periodic Tuning):**
